...
const { argv } = require("yargs")
  ...
  .option("target", {
    ...
    type: "string",
    description:
      "Specify the build target - which file needs to be run once the project has built"
  })
  .option("targetArgs", {
    alias: "a",
    type: "array",
...
async function fetchProjectData(secret, projectName, target, targetArgs) {
  ...
  if (projectData.Count !== 1) {
    projectData = await readProjectData(
      dynamoCredentials,
...
async function main() {
  const { secret, projectName, target, targetArgs } = argv;
